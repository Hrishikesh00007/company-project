...
<div align="right" style="padding:5px;"><form name="frm1" method="post" action="member-inactive.php?act=search"><input type="text" name="search" id="search" class="form-control input-line input-medium" value="<?=$_REQUEST['search']?>" required placeholder="User ID, Name, Phone" style="width:250px;" />
</form></div>
...
{
$where="WHERE (`userid` LIKE '".trim(mysqli_real_escape_string($conn,$_POST['search']))."'  OR `name` LIKE '".trim(mysqli_real_escape_string($conn,$_POST['search']))."'  OR `phone` LIKE '".trim(mysqli_real_escape_string($conn,$_POST['search']))."') AND `paystatus`='I' ORDER BY `id` DESC";
}else{
