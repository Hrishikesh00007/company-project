...
<h4 class="card-title">Fund Request Statement</h4>
...
<div class="row">
<form name="frm1" method="post" action="payment-request.php?act=search">
...
<th width="15%" align="center">Transaction_ID</th>
<th width="10%" align="center">Amount</th>
<th width="15%" align="center">Slip</th>
...
<td align="center" style="padding:5px;"><?=$fetch['tranid']?></td>
...
<td align="center" style="padding:5px;"><?php if($fetch['status']=='P'){?><a href="payment-request-process.php?case=status&id=<?=$fetch['id']?>&st=<?=$fetch['status']?>&amount=<?=$fetch['amount']?>&tranid=<?=$fetch['tranid']?>&page=<?=$_REQUEST['page']?>&userid=<?=$fetch['userid']?>" style="text-decoration:none;"><span style="color:#FFFFFF;background:#FF0000;padding:2px 10px;border-radius:5px;" onClick="return confirm('Are you sure want to change status of payment request?');">Pending</span></a><?php }else{?><span style="color:#FFFFFF;background:#009900;padding:2px 10px;border-radius:5px;">Approved</span><?php }?></td>
