...
<?php 
$sql="SELECT * FROM `ee_settings_company` WHERE `id`='".trim($_REQUEST['id'])."'";
$res=query($conn,$sql);
$num=numrows($res);
if($num>0)
{
...
<form class="form" action="settings-company-process.php?case=edit&id=<?=$_REQUEST['id']?>" method="post" enctype="multipart/form-data">
...
<input type="file" id="file" name="file" class="form-control  border-primary" placeholder="QR Code" accept=".jpg,.png,.jpeg,.pjp"  />                            
</div>
</div>
...
<?php
$tname='ee_settings_company';
$lim=100;
$tpage='settings_company.php';
...
<td style="text-align:center;"><?php if($fetch['qrcode']){?><img src="qrcode/<?=$fetch['qrcode']?>" height="100" width="100" /><?php }?></td>
<td align="center">
<a href="settings-company.php?case=edit&id=<?=$fetch['id']?>" onClick="return confirm('Are you sure want to edit?');"><img src="images/edit.png" /></a>&nbsp;
</td>
</tr>
<?php $i++;}}else{?>
